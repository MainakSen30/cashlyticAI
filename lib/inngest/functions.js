...
function isTransactionDue(transaction) {
  //if no lastProcesseddate, transaction is due
...
async function generateFinancialInsights(stats, month) {
  ...
  try {
    ...
    // Safely extract text output (Gemini SDK versions differ)
    let text =
      result.text ||
    ...
    // Clean up potential extra text around JSON (e.g., "Here are the insights: [...]")
    const jsonMatch = text.match(/\[.*\]/s);
    const jsonText = jsonMatch ? jsonMatch[0] : text;

    return JSON.parse(jsonText);
  } catch (error) {
    console.error("Error generating or parsing Gemini insights:", error);
    return [
